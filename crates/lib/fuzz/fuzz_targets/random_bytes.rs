...
struct FuzzSignTransactionRequest(SignTransactionRequest);

impl<'a> Arbitrary<'a> for FuzzSignTransactionRequest {
    fn arbitrary(u: &mut Unstructured<'a>) -> arbitrary::Result<Self> {
        u.arbitrary::<Vec<u8>>().and_then(|bytes| {
            let encoded = base64::engine::general_purpose::STANDARD.encode(bytes);
            Ok(Self(SignTransactionRequest {
                transaction: encoded,
                signer_key: None,
                sig_verify: true,
            }))
...
impl fmt::Debug for FuzzSignTransactionRequest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let request @ SignTransactionRequest { transaction, signer_key: _, sig_verify: _ } =
            &self.0;
...
fuzz_target!(|data: FuzzSignTransactionRequest| {
    ...
    let request = data.0;
    let _res = pollster::block_on(rpc.sign_transaction(request));
});
