...
impl InitialState {
    pub fn new() -> Self {
        ...
        //let _ = svm.airdrop(&kora_signer.pubkey(), 1_000_000_000);
        ...
        let [spl_metadata] = [spl_id()/* , spl_2022_id()*/].map(|token_program| {
            ...
            let atas= pubkeys.map(|key| {
                    let ata = litesvm_token::CreateAssociatedTokenAccount::new(&mut svm, &kora_signer, &mint)
                        .owner(&key)
                        .send().unwrap();
                    let _ = litesvm_token::MintTo::new(&mut svm, &kora_signer, &mint, &ata, 100_000_000)
                        .send().unwrap();
                    ata
                    });

            TokenMetadata { program: token_program, mint_pubkey: mint, decimals: decimals, atas: atas, is_2022: token_program == spl_2022_id() }
        });
